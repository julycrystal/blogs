...
I needed the Spotify Refresh Token for my blog site in which I could display my [Top 10 Tracks](https://julycrystal.vercel.app/stats) as well as display the currently playing track in the footer section.
...
- Follow the steps from Approach 1 till step 2 and add `<website>/callback` to your `Redirect URIs`. Eg. `http://julycrystal.vercel.app/callback`
...
- This is how mine looked like.
  ...
  ```bash
  https://accounts.spotify.com/authorize?response_type=code&client_id=CLIENT_ID&scope=SCOPE&redirect_uri=https%3A%2F%2Fjulycrystal.vercel.app%2Fcallback
  ```
...
- You will be redirected to your redirect URI which in my case was set to *https://julycrystal.vercel.app/callback*.
...
- In the address bar you will find a huge URL string similar to the one below. In place of `$ACCESSCODE` there will be a long string of characters. Note down that string for the next step.
  ...
  ```bash
  https://julycrystal.vercel.app/callback?code=$ACCESSCODE
  ```
<br />
